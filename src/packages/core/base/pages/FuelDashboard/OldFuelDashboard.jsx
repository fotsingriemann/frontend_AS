...
class FuelDashboard extends Component {
  ...
  mapToArr(devices) {
    ...
    devices.forEach((element) => {
      ...
      rowData.push(
        <a href={startLocationLink} target="_blank">
          {' '}
          {element.eventStartLoc.eventAddress}{' '}
        </a>
      )
      rowData.push(
      ...
      rowData.push(
        <a href={endLocationLink} target="_blank">
          {' '}
          {element.eventEndLoc.eventAddress}{' '}
        </a>
      )
  ...
  getFuelStat = async () => {
    ...
    const response = await this.props.client.query({
      ...
      variables: {
        ...
        timeBegin: parseInt(this.getUnix(this.state.fromDate), 10),
        timeEnd: parseInt(this.getUnix(this.state.toDate), 10),
      },
    })
    ...
    if (response && response.data.getVehicleFuelStatistics.data) {
      ...
      this.setState({
        fuelStatData:
          response.data.getVehicleFuelStatistics.data.vehicleDataList[0].fuel,
      })
    }
  }
                  ...
                  >
                    <Grid item xs={2}>
                      ...
                      >
                        <Paper className={classes.paper}>
                          <Typography variant="h5">
                            {parseFloat(this.state.fuelStatData.draining, 10) /
                              10}
                          </Typography>
                          <Typography variant="body1" gutterBottom>
